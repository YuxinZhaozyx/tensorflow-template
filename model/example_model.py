...
class ExampleModel(BaseModel):
    ...
    def build_model(self):
        ...
        y_onehot = tf.one_hot(self.y, depth=10, axis=-1)
        ...
        with tf.name_scope("loss"):
            self.cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y_onehot, logits=d2))
            ...
            correct_prediction = tf.equal(tf.argmax(d2, 1), tf.argmax(y_onehot, 1))
